...
def define_geometry(config):
    ...
    conn_y_min, conn_y_max = all_roads['I1_NBD']['y_max'], all_roads['I2_SBD']['y_min']
    ix_size_vert_half, sw_width = config['intersection_size'] / 2, config['sidewalk_width']
    bike_lane_w = config.get("bike_lane_width", 0.0)
    ...
    all_sidewalks['CONN_SW_W'] = {'x_min': -bike_lane_w - sw_width, 'x_max': -bike_lane_w, 'y_min': conn_y_min, 'y_max': conn_y_max}
    all_bike_lanes['CONN_BL_W'] = {'x_min': -bike_lane_w, 'x_max': 0, 'y_min': conn_y_min, 'y_max': conn_y_max}
    all_sidewalks['CONN_SW_E'] = {'x_min': 2 * ix_size_vert_half + bike_lane_w, 'x_max': 2 * ix_size_vert_half + bike_lane_w + sw_width, 'y_min': conn_y_min, 'y_max': conn_y_max}
    all_bike_lanes['CONN_BL_E'] = {'x_min': 2 * ix_size_vert_half, 'x_max': 2 * ix_size_vert_half + bike_lane_w, 'y_min': conn_y_min, 'y_max': conn_y_max}
...
def generate_single_intersection_geometry(config, ix_id, center_y, ix_type):
    ...
    ix_size_x, x_w, x_e = 2 * one_way_w_vert, 0, 2 * one_way_w_vert
    ...
    # Extend the N/S crosswalks to span over the bike lanes.
    crosswalks[f'{ix_id}_CW_S'] = {'x_min': x_w - bike_lane_w, 'x_max': x_e + bike_lane_w, 'y_min': y_s_horiz - cw_width, 'y_max': y_s_horiz}
    crosswalks[f'{ix_id}_CW_N'] = {'x_min': x_w - bike_lane_w, 'x_max': x_e + bike_lane_w, 'y_min': y_n_horiz, 'y_max': y_n_horiz + cw_width}
    
    # Shift the E/W crosswalks to be adjacent to the bike lane.
    crosswalks[f'{ix_id}_CW_E'] = {'x_min': x_e + bike_lane_w, 'x_max': x_e + bike_lane_w + cw_width, 'y_min': y_s_horiz, 'y_max': y_n_horiz}
    
    if ix_type != '3_leg_t_no_west':
        crosswalks[f'{ix_id}_CW_W'] = {'x_min': x_w - bike_lane_w - cw_width, 'x_max': x_w - bike_lane_w, 'y_min': y_s_horiz, 'y_max': y_n_horiz}
        
    # Road segments will adjust automatically.
    ...
    # MODIFICATION: Extend straight sidewalks to fill the corners completely.
    # Extend vertical sidewalks (N_W, S_W, N_E, S_E) to meet the horizontal road's edge.
    sidewalks[f'{ix_id}_SW_N_W'] = {'x_min': -bike_lane_w - sw_width, 'x_max': -bike_lane_w, 'y_min': y_n_horiz, 'y_max': crosswalks[f'{ix_id}_CW_N']['y_max'] + road_len}
    sidewalks[f'{ix_id}_SW_S_W'] = {'x_min': -bike_lane_w - sw_width, 'x_max': -bike_lane_w, 'y_min': crosswalks[f'{ix_id}_CW_S']['y_min'] - road_len, 'y_max': y_s_horiz}
    sidewalks[f'{ix_id}_SW_N_E'] = {'x_min': x_e + bike_lane_w, 'x_max': x_e + bike_lane_w + sw_width, 'y_min': y_n_horiz, 'y_max': crosswalks[f'{ix_id}_CW_N']['y_max'] + road_len}
    sidewalks[f'{ix_id}_SW_S_E'] = {'x_min': x_e + bike_lane_w, 'x_max': x_e + bike_lane_w + sw_width, 'y_min': crosswalks[f'{ix_id}_CW_S']['y_min'] - road_len, 'y_max': y_s_horiz}
    
    # Extend horizontal sidewalks (E_N, E_S, W_N, W_S) to meet the vertical road's edge.
    sidewalks[f'{ix_id}_SW_E_N'] = {'x_min': x_e + bike_lane_w, 'x_max': crosswalks[f'{ix_id}_CW_E']['x_max'] + road_len, 'y_min': y_n_horiz, 'y_max': y_n_horiz + sw_width}
    sidewalks[f'{ix_id}_SW_E_S'] = {'x_min': x_e + bike_lane_w, 'x_max': crosswalks[f'{ix_id}_CW_E']['x_max'] + road_len, 'y_min': y_s_horiz - sw_width, 'y_max': y_s_horiz}
    
    # The CORNER patches are no longer needed.
    # DELETED: CORNER_SE, CORNER_NE, CORNER_SW, CORNER_NW definitions.

    if ix_type != '3_leg_t_no_west':
        sidewalks[f'{ix_id}_SW_W_N'] = {'x_min': crosswalks[f'{ix_id}_CW_W']['x_min'] - road_len, 'x_max': x_w - bike_lane_w, 'y_min': y_n_horiz, 'y_max': y_n_horiz + sw_width}
        sidewalks[f'{ix_id}_SW_W_S'] = {'x_min': crosswalks[f'{ix_id}_CW_W']['x_min'] - road_len, 'x_max': x_w - bike_lane_w, 'y_min': y_s_horiz - sw_width, 'y_max': y_s_horiz}
    else: 
        # This part for the T-junction's western side is now consistent with the new corner logic.
        sidewalks[f'{ix_id}_SW_W_MID'] = {'x_min': -bike_lane_w - sw_width, 'x_max': -bike_lane_w, 'y_min': y_s_horiz, 'y_max': y_n_horiz}
        bike_lanes[f'{ix_id}_BL_W_MID'] = {'x_min': -bike_lane_w, 'x_max': 0, 'y_min': y_s_horiz, 'y_max': y_n_horiz}

    # Bike lane definitions do not need changes.
    bike_lanes[f'{ix_id}_BL_N_W'] = {'x_min': -bike_lane_w, 'x_max': 0, 'y_min': crosswalks[f'{ix_id}_CW_N']['y_max'], 'y_max': crosswalks[f'{ix_id}_CW_N']['y_max'] + road_len}
    bike_lanes[f'{ix_id}_BL_S_W'] = {'x_min': -bike_lane_w, 'x_max': 0, 'y_min': crosswalks[f'{ix_id}_CW_S']['y_min'] - road_len, 'y_max': crosswalks[f'{ix_id}_CW_S']['y_min']}
    bike_lanes[f'{ix_id}_BL_N_E'] = {'x_min': x_e, 'x_max': x_e + bike_lane_w, 'y_min': crosswalks[f'{ix_id}_CW_N']['y_max'], 'y_max': crosswalks[f'{ix_id}_CW_N']['y_max'] + road_len}
    bike_lanes[f'{ix_id}_BL_S_E'] = {'x_min': x_e, 'x_max': x_e + bike_lane_w, 'y_min': crosswalks[f'{ix_id}_CW_S']['y_min'] - road_len, 'y_max': crosswalks[f'{ix_id}_CW_S']['y_min']}
...
def get_bike_lane_for_point(x, y):
    for name, bounds in BIKE_LANES.items():
        if bounds['x_min'] <= x < bounds['x_max'] and bounds['y_min'] <= y < bounds['y_max']: return name
    return None
...
def plot_environment(config, ax=None, show_street_names=False, show_road_names=False):
    ...
    road_color, sidewalk_color, bike_lane_color = '#666666', '#B0B0B0', 'seagreen'
    all_x, all_y = [], []
    for collection in [ROADS, SIDEWALKS, CROSSWALKS, BIKE_LANES]:
    ...
    for _, bounds in BIKE_LANES.items():
        ax.add_patch(Rectangle((bounds['x_min'], bounds['y_min']), bounds['x_max'] - bounds['x_min'], bounds['y_max'] - bounds['y_min'], facecolor=bike_lane_color, zorder=0.3))
